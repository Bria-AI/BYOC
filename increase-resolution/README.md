...
- **Python 3.10+**. `torch` is pinned to **2.4.1** and installs automatically with the `[gpu]`/`[all]`
  extra — its default PyPI build targets CUDA 12.1, which covers virtually all modern GPUs (Ampere,
  Ada, Hopper). If your driver doesn't support CUDA 12.1, install a different CUDA build of the same
  2.4.1 version explicitly first (e.g. `--index-url https://download.pytorch.org/whl/cu118`).
...
> minutes, per scale) and the pipeline **warms it up** as part of that same call. This is a one-time
> cost per process: every `execute()` call afterward is fast, since the model is already compiled
> and warm.

## Inputs / config

- **Input** (`IncreaseResolutionInput`): `image` accepts a URL, base64 string, numpy array, or PIL
  image (`bria_external.image.ImageModel`). There is **no `desired_increase` field** — the upscale
  factor is fixed by which model class you construct (`IncreaseResolution2x` / `IncreaseResolution4x`).
- **Config** (`IncreaseResolutionConfig`): `precision` (`"fp32"` default, matching the reference model
  / `"fp16"` for extra speed), `compile_model` (default `True`), `warmup_iters`, `hf_token`,
  `hf_repo_id`, `cache_folder`, tiling params (`tile_size`, `tile_overlap`, `window_size`), and
...
# NOTE: pip resolves the highest-versioned match across ALL configured indexes regardless of
# --index-url/--extra-index-url order -- a same-named public package could still shadow this one.
# bria-increase-res has no PyPI passthrough of its own (needed to resolve public deps like numpy),
# so this is a known residual risk here, not a guarantee.
...
model.setup()                      # downloads model weights -> torch.compile -> warmup (one-time, slow)

result = model.execute(IncreaseResolutionInput(image="https://.../photo.jpg"))  # fast after setup()
...
- The SwinIR architecture is vendored (see `NOTICE`) rather than depended on, since the upstream
  project isn't pip-packaged; only the arch module is vendored — tiling, weight download, config, and
  `torch.compile`/warmup are all this package's own code.
- Weights are the public SwinIR checkpoints (real-world SR, GAN-trained, for both 2× and 4×), re-hosted
  on the gated `briaai/increase-resolution` HF repo for the BYOC download flow.
